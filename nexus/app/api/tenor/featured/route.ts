...
export async function GET(request: NextRequest) {
  ...
  try {
    // Try Giphy first (easiest setup)
    if (GIPHY_API_KEY && GIPHY_API_KEY.length > 0) {
      console.log('[GIF API] Using Giphy provider');
      ...
      if (response.ok) {
        ...
        console.log(`[GIF API] Successfully fetched ${results.length} GIFs from Giphy`);
        return NextResponse.json({ results });
      } else {
        console.error(`[GIF API] Giphy API error: ${response.status} ${response.statusText}`);
      }
    }

    // Try Klipy
    if (KLIPY_API_KEY) {
      const url = new URL("https://api.klipy.com/v1/gifs/trending");
    ...
    console.warn("No GIF API key configured. Set GIPHY_API_KEY or KLIPY_API_KEY.");
    return NextResponse.json({ results: [] });
  } catch (error) {
    console.error("[GIF API] Fatal error:", error);
    if (error instanceof Error) {
      console.error("[GIF API] Error details:", error.message, error.stack);
    }
    return NextResponse.json({ results: [], error: "Failed to fetch GIFs" });
  }
}
